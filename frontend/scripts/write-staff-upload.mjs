...
const content = `import { useState, useRef, useCallback } from 'react';
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    <h5 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                      {doneCount === uploadFiles.length ? (
                        ...
                        <><span style={{ display: 'inline-block', width: 16, height: 16, border: '2px solid #e2e5ea', borderTopColor: '#1a6b4e', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} /> Đang tải lên...</>
                      )}
                    </h5>
                      ...
                      <div key={f.id} style={{
                        ...
                        <div style={{ width: 120, flexShrink: 0 }}>
                          <div style={{ height: 6, background: '#e2e5ea', borderRadius: 3, overflow: 'hidden' }}>
                            <div style={{
                              height: '100%', background: '#1a6b4e', borderRadius: 3,
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ display: 'flex', border: '1px solid #e2e5ea', borderRadius: 8, overflow: 'hidden' }}>
                  {(['grid', 'list'] as const).map((mode) => (
                    <button
                      key={mode}
                      onClick={() => setPhotoView(mode)}
                      style={{
                        padding: '6px 10px', border: 'none', cursor: 'pointer',
                        background: photoView === mode ? '#1a6b4e' : '#fff',
                        color: photoView === mode ? '#fff' : '#8b91a0',
...
const outPath = join(__dirname, '..', 'src', 'pages', 'dashboard', 'StaffUpload.tsx');
writeFileSync(outPath, content, 'utf8');
console.log('✅ Written StaffUpload.tsx');
