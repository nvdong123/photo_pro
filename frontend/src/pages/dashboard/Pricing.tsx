...
export default function Pricing() {
  ...
  const deleteCoupon = (id: number) => {
    Modal.confirm({
      ...
      okType: 'danger',
      cancelText: 'Hủy',
      onOk: () => { setCoupons(p => p.filter(c => c.id !== id)); message.success('Đã xóa mã giảm giá'); },
    });
  };
    ...
    <div>
      ...
      <Table
        ...
        columns={[
          ...
          { title: 'Loại giảm giá', dataIndex: 'type', key: 'type', render: (type: string) => type === 'percent' ? 'Giảm %' : 'Giảm tiền' },
          { title: 'Giá trị', key: 'value', render: (_: unknown, c: Coupon) => <strong>{c.type === 'percent' ? `${c.value}%` : fmt(c.value)}</strong> },
          { title: 'Số lượng', dataIndex: 'quantity', key: 'quantity' },
          { title: 'Đã dùng', dataIndex: 'used', key: 'used' },
          { title: 'Hết hạn', dataIndex: 'expiry', key: 'expiry', render: (v: string) => <span style={{ color: '#5a6170' }}>{v}</span> },
          { title: 'Trạng thái', dataIndex: 'status', key: 'status', render: (status: string) => <Tag color={status === 'active' ? 'green' : 'default'}>{status === 'active' ? 'Đang hoạt động' : 'Hết hạn'}</Tag> },
